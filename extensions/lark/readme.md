...
openclaw config set channels.lark.appId "cli_a9fc5173f1b8ded0"
openclaw config set channels.lark.appSecret "zg3K9mVcKMcGdDBZTrf9Ybr3lKa2YFN6"

# Lark bot openID: Mentioned Only Response

openclaw config set channels.lark.botOpenId "ou_60fbc6f87749c6b6f7816136d1d816b6"
...
openclaw config set channels.lark.oauthRedirectUri "https://warming-evanescence-tone-unions.trycloudflare.com/oauth/callback"

# Oauth权限：

openclaw config set channels.lark.oauthScope "docx:document wiki:wiki:readonly drive:drive:readonly bitable:app"
...
Webhook: Lark开发者平台（Bot App），回调 ->
    ...
    接受信息：群里面有人at Bot ->  (回调) https://warming-evanescence-tone-unions.trycloudflare.com/webhook (CloudFlare Tunnel)
                            ->  转发到 Tencent Cloud 43.162.107.61 （本机）9000端口  -> Lark Extension  -> OpenClaw Channel
                            -> Agent 操作
    Agent操作：
...
2. 增加了lark-mcp skill：(SKILL.md)
   /home/ubuntu/.openclaw/skills/lark-mcp/SKILL.md

完全重新配置：

1. 先Tencent Cloud开一台预装了OpenClaw金属机
2. 增加我的这些改动点；
   extension: lark
   skill: lark-mcp
